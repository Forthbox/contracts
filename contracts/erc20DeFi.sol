...
contract ForthBox_StakingRewards is IStakingRewards, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;

    string private _name = "ForthBox LP DeFi";
    ...
    function stake(uint256 amount) external nonReentrant updateReward(msg.sender){
        require(amount > 0, "ForthBoxNFTDeFi: Cannot stake 0");
        require(isWhiteContract(_msgSender()), "ForthBoxNFTDeFi: Contract not in white list!");
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender){
        require(isWhiteContract(_msgSender()), "ForthBoxNFTDeFi: Contract not in white list!");
        require(amount > 0, "ForthBoxNFTDeFi: Cannot withdraw 0");
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)){
        ...
        require(rewardRate <= balance.div(rewardsDuration), "ForthBoxNFTDeFi:Provided reward too high");
    ...
    function addWhiteAccount(address account) external onlyOwner{
        require(!_Is_WhiteContractArr[account], "ForthBoxNFTDeFi:Account is already White list");
        require(account.isContract(), "ForthBoxNFTDeFi: not Contract Adress");
    ...
    function removeWhiteAccount(address account) external onlyOwner{
        require(_Is_WhiteContractArr[account], "ForthBoxNFTDeFi:Account is already out White list");
